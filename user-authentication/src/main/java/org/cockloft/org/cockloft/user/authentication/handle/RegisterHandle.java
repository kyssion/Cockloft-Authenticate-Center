...
public class RegisterHandle implements Handler<RouteContext> {
    @Override
    public void handle(RouteContext context) {
        HttpServerRequest request = context.getRequest();
        context.getRequest().bodyHandler((buff)->{
            UserRegisterData registerData = Json.decodeValue(buff,UserRegisterData.class);
            ...
            try {
                ...
                e.printStackTrace();
                ResponseData responseData = ResponseData.create(e.getCode(), e.getMsg(), e.getDesc());
                request.response().end(Json.encode(responseData));
            }
        });
    }
